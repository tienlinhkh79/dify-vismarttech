...
class ToolBuiltinProviderUpdateApi(Resource):
    ...
    def post(self, provider):
        ...
        raw_credentials = dict(payload.credentials or {})
        sanitized_credentials = dict(raw_credentials)
        # Compatibility: old UI payloads may include synthetic keys like "0"
        # inside credentials; remove non-schema metadata keys before validation.
        for key in list(sanitized_credentials.keys()):
            if str(key).isdigit() or key in {"credential_id", "id"}:
                sanitized_credentials.pop(key, None)

        credential_id = payload.credential_id
        # Compatibility: some payloads nest metadata under credentials["0"].
        nested_meta = raw_credentials.get("0") if isinstance(raw_credentials.get("0"), dict) else None
        if credential_id in (None, "", "0", 0) and nested_meta:
            credential_id = nested_meta.get("id") or nested_meta.get("credential_id")

        if credential_id in (None, "", "0", 0):
            # Compatibility: some web bundles call update without credential_id.
            existing_credentials = BuiltinToolManageService.get_builtin_tool_provider_credentials(
                tenant_id=tenant_id,
        ...
        logger.info(
            "builtin provider update request provider=%s credential_id=%s raw_credential_keys=%s sanitized_credential_keys=%s elapsed_ms=%.2f",
            provider,
            credential_id,
            list(raw_credentials.keys()),
            list(sanitized_credentials.keys()),
        ...
        logger.info(
            "builtin provider update completed provider=%s credential_id=%s total_elapsed_ms=%.2f",
            provider,
            credential_id,
            (time.perf_counter() - started_at) * 1000,
        )
...
@console_ns.route("/workspaces/current/tool-provider/builtin/<path:provider>/credentials_schema")
class ToolBuiltinProviderCredentialsSchemaLegacyApi(Resource):
    @setup_required
    @login_required
    @account_initialization_required
    def get(self, provider):
