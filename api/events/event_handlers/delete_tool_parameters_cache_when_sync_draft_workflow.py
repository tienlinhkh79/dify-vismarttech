    ...
    for node_data in synced_draft_workflow.graph_dict.get("nodes", []):
        if node_data.get("data", {}).get("type") == BuiltinNodeTypes.TOOL:
            try:
                ...
                # The workflow can reference deleted providers/tools while syncing draft history.
                # Ignore and continue processing other nodes.
                logger.info(
                    "Skip deleting cache for missing tool provider in workflow %s node %s",
                    app.id,
                    node_data.get("id"),
                )
            except Exception:
