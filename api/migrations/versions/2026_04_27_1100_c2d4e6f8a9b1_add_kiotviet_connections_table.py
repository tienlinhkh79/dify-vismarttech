...
def upgrade():
    ...
    if not inspector.has_table("kiotviet_connections"):
        op.create_table(
    ...
    existing_indexes = {index["name"] for index in inspector.get_indexes("kiotviet_connections")}
    if "idx_kiotviet_tenant" not in existing_indexes:
        op.create_index("idx_kiotviet_tenant", "kiotviet_connections", ["tenant_id"], unique=False)
    if "idx_kiotviet_tenant_connection_id" not in existing_indexes:
        op.create_index(
