    ...
    def send_reply(
        ...
        *,
        ...
        if channel_type in (OmniChannelType.FACEBOOK_MESSENGER, OmniChannelType.INSTAGRAM_DM):
            ...
            if not token:
                raise ValueError("Channel access token is missing; reconnect the channel in Settings → Channels")
            ...
            if not ztoken:
                raise ValueError("Zalo OA token is missing; complete OAuth under Settings → Channels")
            if url:
                raise ValueError("Attachments are not supported for Zalo OA in the console inbox yet")
            ...
            ZaloRuntimeService._send_text_reply(external_user_id, body, channel_config=cfg)
        elif channel_type == OmniChannelType.TIKTOK_MESSAGING:
            raise ValueError("Manual replies from the inbox are not implemented for TikTok Messaging yet")
        else:
