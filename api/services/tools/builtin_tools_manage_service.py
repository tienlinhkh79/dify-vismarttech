...
class BuiltinToolManageService:
    ...
    def update_builtin_tool_provider(
        ...
        """
        provider_controller: BuiltinToolProviderController | None = None
        ...
        with Session(db.engine) as session:
            ...
            try:
                if CredentialType.of(db_provider.credential_type).is_editable() and credentials:
                    ...
                    # External credential validation may include network calls.
                    # Do it outside this DB session to avoid long-held transactions.
                    ...
                    encrypted_credentials = json.dumps(encrypter.encrypt(new_credentials))
        ...
        with Session(db.engine) as session:
            ...
            try:
                if encrypted_credentials is not None:
                    db_provider.encrypted_credentials = encrypted_credentials
                    ...
                    if session.scalar(
                        select(
                            exists().where(
                                ...
                                BuiltinToolProvider.name == new_name,
                            )
                        )
                    ):
                        raise ValueError(f"the credential name '{new_name}' is already used")

                    db_provider.name = new_name
        ...
        if encrypted_credentials is not None:
            ToolProviderCredentialsCache(tenant_id=tenant_id, provider=provider, credential_id=credential_id).delete()
        return {"result": "success"}
    ...
    def list_builtin_tools(user_id: str, tenant_id: str) -> list[ToolProviderApiEntity]:
        ...
        def find_provider(provider):
            ...
            provider_full_id = str(ToolProviderID(provider))
            return next(
                filter(
                    lambda db_provider: db_provider.provider in {provider_name, provider_full_id},
                    db_providers,
                ),
                None,
            )
